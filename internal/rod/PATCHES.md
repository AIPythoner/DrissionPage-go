...
3. `page_eval.go`：iframe owner 的 Describe 使用当前操作 context，避免进程切换时继承浏览器生命周期导致无法及时退出。

回归：`TestFrameRebindingAndFileDrop` 验证同源/跨站往返与文件拖入；`TestAsyncHTMLDemo/12_navigation` 验证新标签页打开后的原页操作。迁移依赖升级时需重新核对这些差异。
