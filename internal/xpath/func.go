...
func substringFunc(arg1, arg2, arg3 query) func(query, iterator) interface{} {
	return func(_ query, t iterator) interface{} {
		...
		start := asNumber(t, functionArgs(arg2).Evaluate(t))
		start = math.Floor(start + 0.5)
		end := math.Inf(1)
		if arg3 != nil {
			length := asNumber(t, functionArgs(arg3).Evaluate(t))
			end = start + math.Floor(length+0.5)
		}
...
func translateFunc(arg1, arg2, arg3 query) func(query, iterator) interface{} {
	return func(_ query, t iterator) interface{} {
		...
		for i, r := range []rune(src) {
			...
			replacement := ""
			if i < len(dest) {
				replacement = string(dest[i])
			}
			mapping[r] = replacement
		}
		var result strings.Builder
		for _, r := range str {
